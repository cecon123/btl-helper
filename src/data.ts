...
const scenarioTemplates = {
  register: [
    ...
    ["client", "CLIENT", "RegisterController.java", "Validate form, so khớp password, tạo RegisterRequest; sau khi thành công giữ user ở Register để bấm Back to Login.", "client/src/main/java/com/auction/client/controller/RegisterController.java"],
  ...
  errors: [
    ...
    ["client", "UI", "NotificationManager.java", "Hiển thị toast trong app, xếp chồng theo loại và có progress line tự biến mất.", "client/src/main/java/com/auction/client/util/NotificationManager.java"],
    ["client", "UI", "UiMotion.java", "Cài micro-animation tập trung cho button hover/press/focus mà không đổi handler.", "client/src/main/java/com/auction/client/util/UiMotion.java"],
  ],
  uiPolish: [
    ["client", "FXML", "AppShell.fxml", "Shell có toastHost, disconnected banner và vùng nội dung để notification nằm trong cửa sổ app.", "client/src/main/resources/fxml/AppShell.fxml"],
    ["client", "CONTROLLER", "AppShellController.java", "Khởi tạo toast host, listener notification toàn cục và cập nhật dashboard khi có system notification.", "client/src/main/java/com/auction/client/controller/AppShellController.java"],
    ["client", "UTIL", "NotificationManager.java", "Tạo toast xếp chồng, progress line và xử lý BID_UPDATE/AUCTION_CLOSED/TIME_EXTENDED/SYSTEM_NOTIFICATION.", "client/src/main/java/com/auction/client/util/NotificationManager.java"],
    ["client", "UTIL", "UiMotion.java", "SceneManager gọi UiMotion để cài hover/press/focus-visible cho button sau khi FXML load.", "client/src/main/java/com/auction/client/util/UiMotion.java"],
    ["client", "CSS", "common.css / shell.css", "CSS định nghĩa toast, progress line, shell banner và trạng thái button polish.", "client/src/main/resources/css/common.css"],
  ],
  database: [
  ...
  mavenCi: [
    ["common", "MAVEN", "pom.xml parent", "Quản lý Java 25, dependencyManagement và module reactor.", "pom.xml"],
...
const generatedScenarioFlows: ScenarioFlow[] = Object.entries(scenarioTemplates).map(([id, rows]) => {
  const titles: Record<string, [string, string, string]> = {
    ...
    uiPolish: ["UI polish", "Luồng toast và micro-interaction", "Toast trong app, progress line, disconnected banner và UiMotion sau FXML load."],
...
export const criticalFiles: CriticalFile[] = [
  ...
  },
  {
    ...
    layer: "Client UI",
    purpose: "Điểm tập trung hiển thị toast và phản ứng với event realtime ở cửa sổ JavaFX.",
    explain: [
      "Toast nằm trong app window, có progress line và xếp chồng theo loại thông báo.",
      "Listener BID_UPDATE, AUCTION_CLOSED, TIME_EXTENDED và SYSTEM_NOTIFICATION phải cập nhật UI đúng JavaFX thread.",
      "Nếu sửa sai, user có thể không thấy lỗi thành công/thất bại dù server xử lý đúng.",
    ],
    linkedTheory: ["Observer", "JavaFX thread", "Realtime event"],
    tests: ["SocketClientIntegrationTest", "ClientHandlerIntegrationTest"],
    risk: "Medium",
  },
  {
    path: "client/src/main/java/com/auction/client/util/UiMotion.java",
    layer: "Client UI",
    purpose: "Cài micro-animation tập trung cho button hover, press và focus-visible.",
    explain: [
      "SceneManager gọi helper này sau khi load FXML để tránh lặp animation code trong từng controller.",
      "Helper chỉ can thiệp style/effect/scale của button, không đổi onAction hay luồng nghiệp vụ.",
      "Đây là phần polish UI mới sau khi chuyển lên JavaFX 25, nên cần biết để demo giao diện mượt mà vẫn an toàn.",
    ],
    linkedTheory: ["JavaFX UI state", "Separation of Concerns"],
    tests: ["IkonliIconLiteralTest"],
    risk: "Low",
  },
  {
    path: "pom.xml",
    layer: "Build",
    purpose: "Parent Maven quản lý version Java 25, module common/server/client và plugin.",
    explain: [
      "maven.compiler.release đặt Java 25; client dùng JavaFX 25 nên máy demo cần JDK 25.",
...
export const theoryTopics: StudyTopic[] = [
  ...
  },
  {
    ...
    links: [{ label: "Java Socket API", url: "https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/net/Socket.html" }],
    status: "review",
  },
  {
    id: "http-assets",
  ...
  },
  {
    ...
    links: [{ label: "JavaFX FXML", url: "https://openjfx.io/javadoc/25/javafx.fxml/javafx/fxml/doc-files/introduction_to_fxml.html" }],
    status: "review",
  },
  {
    id: "dto-protocol",
  ...
  },
  {
    ...
    links: [{ label: "ScheduledExecutorService", url: "https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/ScheduledExecutorService.html" }],
    status: "risk",
  },
  {
    id: "realtime",
  ...
  },
  {
    ...
    category: "Build",
    level: "Core",
    summary: "Parent pom quản lý Java 25; module common/server/client build theo reactor và chia dependency đúng phạm vi. Lệnh demo chính là `mvn clean package` chạy tại thư mục gốc.",
    projectExample: "`mvn clean package` xóa target cũ, build common trước rồi server/client, chạy test và tạo artifact; client dùng JavaFX 25 nên máy demo cần JDK 25.",
  ...
  },
  {
    ...
    category: "Frontend",
    level: "Advanced",
    summary: "JavaFX controller phải giữ UI phản hồi nhanh, cập nhật đúng thread, hiển thị toast rõ ràng và không để animation phá handler.",
    projectExample: "NotificationManager hiển thị toast trong app; UiMotion cài hover/press/focus cho button; AuctionStatusUi và BidTimeline chuẩn hóa trạng thái.",
    files: ["NotificationManager.java", "UiMotion.java", "AuctionStatusUi.java", "PriceChartManager.java"],
    links: [{ label: "JavaFX threading", url: "https://openjfx.io/javadoc/25/javafx.graphics/javafx/application/Platform.html" }],
    status: "review",
  },
];
